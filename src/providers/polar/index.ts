...
/**
 * Migrate products from a Polar.sh organization into a Dodo Payments brand.
 *
 * Fetches all products for the given Polar organization, transforms Polar price variants
 * into Dodo-compatible product entries (one Dodo product per Polar price variant),
 * presents a preview, prompts for confirmation, and creates the selected products in
 * Dodo Payments. Logs a summary of successes and failures. Exits the process on Polar
 * rate-limit (HTTP 429) responses.
 *
 * @param polar - Initialized Polar SDK client used to list products
 * @param client - Initialized Dodo Payments client used to create products
 * @param organization_id - Polar organization ID to migrate products from
 * @param brand_id - Dodo Payments brand ID to assign migrated products to
 */
...
/**
 * Migrate percentage-based discounts from Polar.sh into Dodo Payments for a given organization and brand.
 *
 * Fetches discounts from the specified Polar organization, filters out expired, fixed-amount, unsupported, or product-restricted discounts (logging warnings where applicable), converts supported percentage discounts (kept in basis points) into Dodo-compatible objects, shows a preview, prompts for user confirmation, and creates the discounts in Dodo Payments while summarizing successes and failures.
 *
 * @param organization_id - Polar organization ID to fetch discounts from
 * @param brand_id - Dodo Payments brand ID to associate created discounts with
 */
...
/**
 * Migrate customers from a Polar.sh organization into a Dodo Payments brand.
 *
 * Fetches customers from the specified Polar organization, filters out records that
 * are missing an email or marked deleted, and maps each remaining customer to the
 * Dodo customer schema (including address and metadata linking back to Polar).
 * Presents a preview and prompts for confirmation before creating customers in the
 * target Dodo brand, then attempts to create each customer and logs a migration summary.
 * If the Polar API returns a 429 rate-limit response, the function logs the retry-after
 * value and exits the process.
 *
 * @param organization_id - The Polar organization ID to fetch customers from
 * @param brand_id - The Dodo Payments brand ID to assign to created customers
 */
